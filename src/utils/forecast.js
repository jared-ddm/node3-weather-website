...
const forecast = (latitude, longitude, callback) => {
    ...
    request({ url, json: true }, (error, {body}) => {
        if (error) {
            ...
            const summary = body.daily.data[0].summary + ` It is currently ${body.currently.temperature} degrees out. There is a ${(body.daily.data[0].precipProbability * 100)}% chance of ${body.daily.data[0].precipType} throughout the day. High: ${Math.round(body.daily.data[0].temperatureHigh)}, Low: ${Math.round(body.daily.data[0].temperatureLow)},  UV-Index: ${body.daily.data[0].uvIndex}`
